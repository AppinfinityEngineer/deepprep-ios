...
async def create(body: FreeScanCreateIn, request: Request):
    try:
        report = await free_scan_service.create_free_scan(
            body.deviceId, body.company, body.role, body.jdText, body.date, body.interviewers,
            _client_ip(request), body.userAgent or request.headers.get("user-agent"), body.attestToken,
        )
